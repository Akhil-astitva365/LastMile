...
3. **⭐ Role-Based Authentication & Access Control (RBAC)**:
   ...
   - **`ADMIN`**: Live platform telemetry, rate card & COD surcharge manager, zone manager, agent monitoring, order filtering, manual agent assignment, and auto-assign trigger.
...
7. **💎 Premium Glassmorphism UI & Clean Location Autocomplete**:
   - Google Font `Syne` logo header, borderless glass card system (`border: none !important`), and real-time Pan-India location search without awkward `Custom Location` prefixes.
...
Weight Charge: ₹180 | Distance Charge: ₹425 | COD Surcharge: ₹30 ➔ Total: ₹635
                     │
                     ▼
